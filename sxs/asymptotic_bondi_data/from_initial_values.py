...
def from_initial_values(cls, time, ell_max=8, sigma0=0.0, sigmadot0=0.0, sigmaddot0=0.0, psi2=0.0, psi1=0.0, psi0=0.0):
    ...
    direction, we can find the values of sigma and all the Weyl components for
    all time -- the Bondi data.  This function implements that algorithm.

    Note that the interpretation of the input mode weights depends on the
    dimension of `sigma0`.  If it has dimension 0 or 1, the problem is
    integrated exactly, and the rest of the inputs are assumed to represent the
    corresponding values at time u=0.  If it has dimension 2, `sigma0` is
    interpreted as a function of time, and the rest of the problem is integrated
    numerically via splines, with `psi2`, `psi1`, and `psi0` representing the
    corresponding values at time u=`time[0]` (the first element of the input
    `time` array); `sigmadot0` and `sigmaddot0` are ignored in this case.
    ...
    sigmaddot0: 0.0 or array_like [defaults to 0.0]
        ...
        represents the second derivative only at time u=0; for all other times
        we expand as a Taylor series if this is used.
